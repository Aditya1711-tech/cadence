...
2026-06-27  P1-A.9  done   service lifecycle (agent/dist): launchd LaunchAgent (RunAtLoad+KeepAlive, Background/LowPriorityIO) + systemd user unit (Restart=on-failure, Nice/CPUWeight/MemoryMax, enable-linger for survive-logout) + install.sh/uninstall.sh (build/locate binary, substitute paths, enable) + README; bash -n + plist XML validated; runtime install/restart verification deferred to mac/linux (Windows out of scope per task); commit <pending>
```
