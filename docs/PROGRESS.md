...
2026-06-27  P1-A.8  done   redaction list (internal/redact): user regex -> SHA-256 hash of matching title/url before store (stable token, groupable); user JSON via CADENCE_REDACT_PATH (scaffold empty), default off; refactored api.New to Options{Classifier,Redactor,Logger}; runs after classify so categories use real values; tests + ingest redaction test green; commit <pending>
```
