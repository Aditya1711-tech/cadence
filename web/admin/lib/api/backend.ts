...
// through here, inside BFF route handlers — so this is the one place that owns
// the access token, the Bearer header, and the invisible refresh-on-401 +
// cookie rotation. The browser never participates in token lifecycle.
...
/** Unauthenticated call to the backend (login, register, invite preview/accept). */
